...
class SystemInistService {
    ...
    private void initConfigs() {
        // 1. 读取数据库全部配置信息
        List<SystemEntity> systemEntities = systemConfigService.findAll().orElseGet(()->Arrays.asList());
        Map<String, String> systemConfigs = new HashMap<>();
        for (SystemEntity item : systemEntities) {
            systemConfigs.put(item.getKeyName(), item.getKeyValue());
        }

        // 2. 分析DEFAULT_CONFIGS
        for (Map.Entry<String, String> entry : DEFAULT_CONFIGS.entrySet()) {
            if (systemConfigs.containsKey(entry.getKey())) {
                continue;
            }

            systemConfigs.put(entry.getKey(), entry.getValue());
            systemConfigService.addConfig(entry.getKey(), entry.getValue());
        }

        SystemConfig.setConfigs(systemConfigs);
    }
